...
(
 ...
 where v.is_accounting_voucher is true and v.date < parse_date('%d-%m-%Y', @fromDate)
 group by a.ledger
),
tblcurr as
(
 ...
 from tallydb.trn_accounting a
 join tallydb.trn_voucher v on v.guid = a.guid
 where v.is_accounting_voucher is true and v.date between parse_date('%d-%m-%Y', @fromDate) and parse_date('%d-%m-%Y', @toDate)
 group by a.ledger
)
select l.name,
