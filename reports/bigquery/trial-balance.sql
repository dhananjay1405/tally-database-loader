...
(
 ...
 where v.is_accounting_voucher is true and v.date < parse_date('%d-%m-%Y', '01-04-2020')
 group by a.ledger
),
tblcurr as
(
 ...
 from tallydb.trn_accounting a
 join tallydb.trn_voucher v on v.guid = a.guid
 where v.is_accounting_voucher is true and v.date between parse_date('%d-%m-%Y', '01-04-2020') and parse_date('%d-%m-%Y', '31-03-2021')
 group by a.ledger
)
select l.name,
