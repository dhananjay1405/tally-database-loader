...
(
 ...
 where lower(g.primary_group) = 'cash-in-hand' and lower(t.parent) in ('receipt', 'payment', 'contra') and (v.date between parse_date('%d-%m-%Y', '01-04-2014') and parse_date('%d-%m-%Y', '31-03-2015'))
 group by v.date
)
select d.dt date, ifnull(c.receipt, 0) receipt, ifnull(c.payment, 0) payment
from (select * from unnest(generate_date_array(parse_date('%d-%m-%Y', '01-04-2014'), parse_date('%d-%m-%Y', '31-03-2015'), interval 1 day)) dt) d
