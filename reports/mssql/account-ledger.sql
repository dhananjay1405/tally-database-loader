/* Account Statement */
declare @ledger nvarchar(1024) = 'Cash', @fromDate date = '2014-04-01', @toDate date = '2015-03-31';
with tblLedger as
(
 ...
 where a.ledger = @ledger and v.date between @fromDate and @toDate
),
tblEntry as
(
 ...
 where v.guid in (select distinct guid from tblLedger)
 group by v.guid
)
select l.*, c.ledgers ledgers
from tblLedger l
