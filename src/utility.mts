...
const weeksShort: string[] = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
const weeksLong: string[] = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
...
class _utility {
    String = {
         ...
         */
        strip(val: string, count = 1): string {
            return val.substr(0, val.length - count);
        },

        insertAt(value: string, index: number, expr: string): string {
         ...
         */
        split(value: string, separator: string, removeEmpty = false): string[] {
        ...
        unescapeHTML(iHtmlStr: string): string {
            let retval = iHtmlStr;
    ...
    Number = {
        ...
        format(value: number, mask: string): string {


            function formatNumber(mask: string, value: number): string {
                ...
                if (mskFraction.length) retval += '.' + numFraction.toFixed(mskFraction.length).substr(2);
                return retval;
            };
    ...
    Date = {
        ...
        dateReviver(key: any, value: any): any {
            if (typeof value === 'string' && dateExpression.test(value))
                return new Date(value);
            return value;
        },

        format(value: Date, mask: string): string {
            ...
            else if (/yy/.test(mask)) mask = mask.replace('yy', vYear.toString().substr(2, 3));
            else;

            //Month replacement
            if (/MMMM/.test(mask)) mask = mask.replace('MMMM', monthsLong[vMonth]);
            else if (/MMM/.test(mask)) mask = mask.replace('MMM', monthsShort[vMonth]);
            else if (/MM/.test(mask)) mask = mask.replace('MM', (vMonth + 1).toString().padStart(2, '0'));
            else if (/M/.test(mask)) mask = mask.replace('M', (vMonth + 1).toString());
            else;

            //Day replacement
            if (/dddd/.test(mask)) mask = mask.replace('dddd', weeksLong[vWeek]);
            else if (/ddd/.test(mask)) mask = mask.replace('ddd', weeksShort[vWeek]);
            else if (/dd/.test(mask)) mask = mask.replace('dd', vDay.toString().padStart(2, '0'));
            else if (/d/.test(mask)) mask = mask.replace('d', vDay.toString());
            else;

            //Hour replacement
            if (/HH/.test(mask)) mask = mask.replace('HH', vHour.toString().padStart(2, '0'));
            else if (/H/.test(mask)) mask = mask.replace('H', vHour.toString());
            else if (/hh/.test(mask)) mask = mask.replace('hh', (vHour % 12 ? 0 : vHour % 12).toString().padStart(2, '0'));
            else if (/h/.test(mask)) mask = mask.replace('h', (vHour % 12 ? 0 : vHour % 12).toString());
            else;

            //Minute replacement
            if (/mm/.test(mask)) mask = mask.replace('mm', vMinute.toString().padStart(2, '0'));
            else if (/m/.test(mask)) mask = mask.replace('m', vMinute.toString());
            else;

            //Second replacement
            if (/ss/.test(mask)) mask = mask.replace('ss', vSecond.toString().padStart(2, '0'));
            else if (/s/.test(mask)) mask = mask.replace('s', vSecond.toString());
            else;

            //AM-PM replacement
            if (/tt/.test(mask)) mask = mask.replace('tt', Math.floor(vHour / 12) ? 'PM' : 'AM');
            else;
    ...
    Object = {
        copyPropertyValues(source: any, target: any): void {
    ...
    Array = {
        distinct(source: any[], field: string): any[] {
            let retval: any[] = [];
        ...
        unique<T, K extends keyof T>(arr: T[], prop: K): T[] {
            let retval: T[] = [];
