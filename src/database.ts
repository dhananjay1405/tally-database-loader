...
class _database {
    ...
    executeNonQuery(sqlQuery: string, values?: Map<string, any>): Promise<number> {
        return new Promise<number>(async (resolve, reject) => {
            try {
                let retval = 0;
    ...
    private executeMysql(sqlQuery: string): Promise<queryResult> {
    ...
    private executeMssql(sqlQuery: string): Promise<queryResult> {
        return new Promise<queryResult>((resolve, reject) => {
            try {
    ...
    private executePostgres(sqlQuery: string): Promise<queryResult> {
